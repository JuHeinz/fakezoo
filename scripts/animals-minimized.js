...
function animals() {
    ...
    ///create an array, that produces 4 random values
        
    var statArray = [];
    var generatedStats = generateStats();
    statArray.push(generatedStats);
    
    function generateStats() {
        for (let x = 0; x < 4; x++) {
            let stat = Math.floor(Math.random() * 101)
            return stat
        }
    }
    ...
    function basics() {
            ...
            var size = statArray[1]
            idSize[i].innerHTML = size + "%";

            var immu = statArray[2]
            idImmu[i].innerHTML = immu + "%";

            var fert = statArray[3]
            idFert[i].innerHTML = fert + "%";

            var life = statArray[4]
