...
var placeOverlay = document.querySelectorAll('.placeOverlay');
...
function info() {
    ...
    else {verdict = "While" + animList[i].name + "'s genetics may not contribute to a healthy re-population of the " + animList[i].species + ", hopefully " + pronoun +  " presence can still bring joy to you!";
    }
    ...
    // PICTURE
    // for (let p = 0; p < anim.length; p++) {
    //     pic[p].src="./images/animals/"+[p]+".jpg"}
    }
...
function adoptionEffect(id) {
    ...
    appearText[id].classList.add("appear");
